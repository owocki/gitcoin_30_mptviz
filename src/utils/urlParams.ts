...
export function updateURLWithConfig(config: SceneConfig): void {
  ...
  const newUrl = `${window.location.origin}/create?cfg=${encoded}`;
...
/**
 * Extract config from a URL string (supports both absolute and relative URLs)
 */
export function getConfigFromURLString(urlString: string): Partial<SceneConfig> | null {
  try {
    // URL constructor handles both absolute URLs and relative URLs with base parameter
